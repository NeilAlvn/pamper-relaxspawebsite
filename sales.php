...
processLogin($conn);
    ...
    if (isset($_GET['action']) && isset($_GET['id'])) {
        ...
        header("Location: sales.php?filter=" . urlencode($filter) . "&search=" . urlencode($search));
        exit();
    }
    ...
    if (!empty($search)) {
        $query .= " AND (name LIKE '%" . $conn->real_escape_string($search) . "%' 
                    ...
                    OR service LIKE '%" . $conn->real_escape_string($search) . "%')";
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadModal(startDate, endDate) {
        ...
        fetch(`modal_results.php?start_date=${encodeURIComponent(startDate)}&end_date=${encodeURIComponent(endDate)}`, {
            method: 'GET',
            headers: {
...
<!-- Date and Time --> 
  ...
  document.addEventListener('click', function(e) {
      const img = e.target.closest('.openProductModal');
      if (!img) return;

      const orderId = img.dataset.order;
      if (!orderId) return;

      // Prevent opening multiple modals
      if (document.getElementById('product-image-modal')) return;

      fetch(`modal_sales_product.php?order=${encodeURIComponent(orderId)}`)
  ...
  <main class="pt-24 pb-16 min-h-screen">
    <div class="max-w-7xl mx-auto px-6 lg:px-12">
      ...
      <div class="text-center mb-12">
        ...
        <p class="text-gray-600 max-w-2xl mx-auto">
          Manage and track all customer products sales with ease.
        </p>
      </div>
    ...
    <div class="flex justify-center items-center gap-6 w-full pb-10">
      <div class="bg-white rounded-xl shadow-md p-6 border border-spaAccent/30">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-3xl font-bold text-yellow-600 mt-1"><?php echo $stats['pending_amount']; ?></p>
            </div>
            <div class="bg-yellow-50 rounded-full p-3">
              <svg class="w-8 h-8 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
        ...
        <div class="bg-white rounded-xl shadow-md p-6 border border-spaAccent/30">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-3xl font-bold text-green-600 mt-1"><?php echo $stats['approved_amount']; ?></p>
            </div>
            <div class="bg-green-50 rounded-full p-3">
              <svg class="w-8 h-8 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
        ...
        <div class="bg-white rounded-xl shadow-md p-6 border border-spaAccent/30">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-3xl font-bold text-red-600 mt-1"><?php echo $stats['rejected_amount']; ?></p>
            </div>
            <div class="bg-red-50 rounded-full p-3">
              <svg class="w-8 h-8 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"/>
      ...
      <div class="bg-white rounded-xl shadow-md p-6 mb-8 border border-spaAccent/30">
        <form method="GET" action="sales.php" class="flex flex-col md:flex-row gap-4">
          ...
          <table class="w-full">
            <thead class="bg-spaGreen text-white">
              <tr>
                ...
                <th class="px-6 py-4 text-left text-sm font-semibold uppercase tracking-wider">Products</th>
              ...
              <?php if ($result->num_rows > 0): ?>
                <?php while($row = $result->fetch_assoc()): ?>
                  <tr class="hover:bg-spaIvory/50 transition">
                    ...
                    <td class="px-6 py-4">
                      <div class="text-sm text-gray-700"><?php echo htmlspecialchars($row['customer_email']); ?></div>
                      <div class="text-sm text-gray-500"><?php echo htmlspecialchars($row['customer_phone']); ?></div>
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4">
                      <span class="text-sm font-semibold text-spaGreen">₱<?php echo number_format($row['total_amount'], 2); ?></span>
                    </td>
                    <td class="px-6 py-4">
                        <?php if(!empty($row['payment_proof']) && file_exists($row['payment_proof'])): ?>
                            <img 
                                src="<?php echo $row['payment_proof']; ?>" 
                                data-order="<?php echo $row['id']; ?>" 
                                class="w-16 h-16 object-cover rounded-lg border-2 border-spaGold hover:scale-110 transition cursor-pointer openProductModal"
                                alt="Payment Proof"
                                title="Click to view"
                            >
                        <?php else: ?>
                    ...
                    <td class="px-6 py-4">
                      <div class="flex items-center space-x-2">
                        <?php if ($status === 'pending'): ?>
                          <a href="sales.php?action=approve&id=<?php echo $row['id']; ?>&filter=<?php echo $filter; ?>&search=<?php echo urlencode($search); ?>" 
                          ...
                          <a href="sales.php?action=reject&id=<?php echo $row['id']; ?>&filter=<?php echo $filter; ?>&search=<?php echo urlencode($search); ?>" 
                        ...
                        <?php endif; ?>
                        <a href="sales.php?action=delete&id=<?php echo $row['id']; ?>&filter=<?php echo $filter; ?>&search=<?php echo urlencode($search); ?>" 
...
<script>
document.addEventListener('click', function(e) {
    if (e.target && e.target.id === 'printModalBtn') {
        const modal = document.getElementById('results-modal');
        if (!modal) return;
